...
final class InventorySet implements Inventory {
	...
	public Iterator<Record> iterator(Comparator<Record> comparator) {// with comparator
		// TODO
		List<Record> tosort = new ArrayList<>(_data.values());
		Collections.sort(tosort, comparator);
		return Collections.unmodifiableCollection(tosort).iterator();
	}
	...
	Record checkOut(Video video) {
		// TODO
		if (!_data.containsKey(video)) { // if don't have it
			throw new IllegalArgumentException();
		}
		RecordObj cur = (RecordObj) _data.get(video);
		if (cur == null || cur.numOut == cur.numOwned) {// if equal, or in same case, a null value for that key
			throw new IllegalArgumentException();
		}
		// replace the old video with new values
		RecordObj newone = new RecordObj(video, cur.numOwned, cur.numOut + 1, cur.numRentals + 1);
		_data.put(video, newone);

		return cur; // return the old one back to CmdOut
	}
	...
	Record checkIn(Video video) {
		...
		RecordObj cur = (RecordObj) _data.get(video);
		if (cur == null || cur.numOut < 1) {// if equal, or in same case, a null value for that key
			throw new IllegalArgumentException();
		}
		// replace the old video with new values
		RecordObj newone = new RecordObj(video, cur.numOwned, cur.numOut - 1, cur.numRentals);
		_data.put(video, newone);
		return cur; // return the old one back to CmdIn
	}
	...
	Map clear() {
		// TODO
		HashMap<Video, Record> previous = new HashMap<Video, Record>(_data);
		_data.clear();
		return previous;
	}
